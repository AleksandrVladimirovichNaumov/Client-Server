...
class ClientVerifier(type):
    def __init__(self, class_name, base_classes, class_dict):
            ...
            try:
                ...
                for i in ret:
                    if i.opname == 'LOAD_GLOBAL' and i.opname not in methods:
                        ...
                        attrs.append(i.argval)
                    # так как send_message & get_message являются методами родительского класса MySocket,
        ...
        if 'accept' in methods or 'listen' in methods or 'socket' in methods:
            client_logger.critical('методы accept / listen / socket ошибочно используется при работе клиента')

        if not 'send_message' in methods or not 'get_message' in methods:
